...
psi' (cont : stack) astack heap atom lbs settings = case cont of
    CtOLet _ | isKnownStack astack heap atom && not (caseBranches $ head settings)
        -> psi (RPsi "remove letbinding") stack (popArg astack) heap (lookupAtomStack  astack atom) lbs settings
